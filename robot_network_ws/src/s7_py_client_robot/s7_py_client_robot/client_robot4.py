...
class PosePublisher(Node):
    def __init__(self):
        ...
        # 6: request new poses
        # 7: wait for connection to the server node (emergency stop)
        self.stage = 0
    ...
    def publish(self):
        # --- Wait for connection to the server node (emergency stop) ---
        while not self.client.wait_for_service(timeout_sec=1.0):
            if not rclpy.ok():
                self.get_logger().error('Interrupted while waiting for service. Exiting.')
                return
