...
def main(args=None):
    ...
    if len(sys.argv) != 4:
        print("Usage: py_client_exe a b c")
        sys.exit(1)
    py_client = PyClientAsync()
    future = py_client.send_request(int(sys.argv[1]), int(sys.argv[2]), int(sys.argv[3]))
    ...
    if result is not None:
        ...
        py_client.get_logger().error('Failed to call service add_three_ints')
